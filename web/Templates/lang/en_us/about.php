...
return $page = [
    'mainHeader' => 'About',
    'introBlock' => '<strong>Core Framework</strong> is a Open Source MVC framework for developing Web 2.0 applications in PHP. This framework was build from ground up in PHP 5.4+ and incoperates the latest features and conventions.
        <br/><br/>
        This framework was started by Shalom Sam in late 2013 and was build to be completely object-oriented, while utilizing most of the new features of PHP 5.4.
        <br/><br/>
        Core Framework was build to be simple and easy to use, and comes with a demo application already setup, making it easy for you the developer to start building your application right away.
        <br/><br/>
        Core Framework is build as components that can be used individually in any other application. The main components that make up Core Framework are Router, BaseController, AppView and DI (Dependency Injection) components. The Router component determines which controller to load based on the current URI and the provided routing instruction. The BaseController component performs the required set of instruction before loading the child controller mapped to a URI. The AppView component is a wrapper around the templating Engine, which currently is Smarty (Support for other templating Engines will come soon). And finally the DI component or Dependency Injection component is responsible for laoding all components or classes seamlessly. DI can also be used to load your on components into your application as and when required.',
